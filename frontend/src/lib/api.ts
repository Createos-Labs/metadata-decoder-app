...
export const api = {
  ...
  getMappingStatus: (acqId: string) =>
    request<import("./types").MAMappingStatus>(`/api/ma/acquisitions/${acqId}/mapping/status`),
  ...
  downloadCurrentMapping: async (acqId: string, acqName: string): Promise<void> => {
    const headers = new Headers();
    if (authToken) headers.set("Authorization", `Bearer ${authToken}`);
    const res = await fetch(`/api/ma/acquisitions/${acqId}/mapping`, { headers });
    if (!res.ok) {
      let detail = res.statusText;
      try { const b = await res.json(); detail = b.detail ?? detail; } catch { /* non-JSON */ }
      throw new ApiError(res.status, detail);
  ...
  resetMapping: (acqId: string) =>
    request<void>(`/api/ma/acquisitions/${acqId}/mapping`, { method: "DELETE" }),
